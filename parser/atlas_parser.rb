...
#Author: August Beers
#The script reads through each of the line of the Atlas text file
#it searchs for events, years, page numbers, and locations

$line = 0

#file to output location data
$location_test_file = File.new('LOC_test.txt', 'w:utf-8')

#The event class encapsulats all the informatin related to an event
#events corrisond to entries in the atlas
class Event
...
#The words can be used to link together proper nouns
$connectors = [
    /di/,
    /del/,
    /della/,
...
#These words corrispond to locations
$locations = [
...
#Erray to hold the events generated by parsing
$events = Array.new
#the current event being written this variable acts as a buffer
...
#a boolean used for looking ahead in the atlas
$line_flip = false

#Boolean to control jumps in the page numberings
#this boolean is nessisary to account for situations where page numbers jump caused by changing 
#pdfs in the atllas
$flip = true
  ...
  if(/\A\d+\Z/.match(line))
    ...
    if(test_page == line.to_i)
      ...
      if ($current_page == '258' && $flip == true)
        ...
        #print current page to keep track of progress
      puts $current_page

        #if not a page number possible a year
